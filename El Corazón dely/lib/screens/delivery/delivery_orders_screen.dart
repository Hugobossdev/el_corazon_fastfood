...
class _DeliveryOrdersScreenState extends State<DeliveryOrdersScreen>
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      ...
      // Recharger aussi l'historique, sinon l'onglet "Terminées" reste vide après refresh.
      await Future.wait([
        // IMPORTANT: écran "Mes Livraisons" = seulement les commandes assignées au livreur.
  ...
  Widget _buildCompletedDeliveries() {
    ...
    return Consumer<AppService>(
      builder: (context, appService, child) {
        // Historique = source DB (driverOrderHistory), pas seulement les commandes actives.
        final completedDeliveries = appService.driverOrderHistory
            .where(
              (order) =>
                  order.status == OrderStatus.delivered ||
                  order.status == OrderStatus.cancelled,
...
class _DeliveryDetailsSheetState extends State<DeliveryDetailsSheet> {
  ...
  void initState() {
    super.initState();
    _locationFuture =
        geo.GeocodingService().geocodeAddress(widget.order.deliveryAddress);
  }
                          ...
                          markers: {
                            Marker(
                              ...
                              infoWindow: InfoWindow(
                                  title: widget.order.deliveryAddress),
                            ),
                          },
  ...
  Future<void> _openNavigation() async {
    try {
      ...
      final canLaunchGoogleMaps = await launcher.canLaunchUrl(googleMapsUri);

      if (canLaunchGoogleMaps) {
        await launcher.launchUrl(googleMapsUri,
            mode: launcher.LaunchMode.externalApplication);
      } else {
        ...
        final canLaunchWaze = await launcher.canLaunchUrl(wazeUri);

        if (canLaunchWaze) {
          await launcher.launchUrl(wazeUri,
              mode: launcher.LaunchMode.externalApplication);
        } else {
          if (mounted && context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
