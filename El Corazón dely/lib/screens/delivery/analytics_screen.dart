  ...
  Widget _buildStatisticsTab() {
    return Consumer<AppService>(
      builder: (context, appService, child) {
        // Stats doivent venir de la DB (historique), pas uniquement des commandes actives en mémoire.
  ...
  Widget _buildOperationTimes(PerformanceService performanceService) {
    ...
    if (operations.isEmpty) {
      ...
      return AppCard(
        child: Column(
          children: [
            Icon(Icons.timer_outlined, size: 40, color: scheme.onSurfaceVariant),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              color: duration != null ? scheme.secondary : scheme.onSurfaceVariant,
              fontWeight: FontWeight.bold,
            ),
          ),
        ],
    ...
    return AppCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.md),
          _buildMetricBar('Efficacité', performanceService.efficiencyScore, scheme.primary),
          const SizedBox(height: AppSpacing.sm),
          _buildMetricBar('Fiabilité', performanceService.reliabilityScore, scheme.secondary),
          const SizedBox(height: AppSpacing.sm),
          _buildMetricBar('Rapidité', performanceService.speedScore, scheme.tertiary),
        ],
      ),
    );
  }
