...
class Promotion {
  ...
  bool get isAvailable =>
      isActive && !isExpired && (usageLimit == -1 || usageCount < usageLimit);
  ...
  double calculateDiscount(double orderAmount) {
    ...
    switch (discountType) {
      ...
      case 'free_delivery':
        // Annule le frais de livraison courant (calculé côté app).
  ...
  Promotion? getPromotionByCode(String code) {
    try {
      return _promotions.firstWhere(
        (promo) =>
            promo.promoCode.toLowerCase() == code.toLowerCase() &&
            promo.isAvailable,
      );
    } catch (e) {
      return null;
  ...
  List<Promotion> getPromotionsForCategory(String category) {
    return _promotions
        .where(
          (promo) =>
              promo.isAvailable &&
              (promo.applicableCategories.isEmpty ||
                  promo.applicableCategories.contains(category)),
        )
        .toList();
  }
