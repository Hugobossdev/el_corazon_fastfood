...
class AgoraCallService extends ChangeNotifier {
  ...
  // NOTE: ApiConfig.agoraAppId provient de dotenv (runtime), donc ne peut pas être `const`.
  ...
  Future<bool> joinChannel({
    ...
    try {
      ...
      // Rejoindre le canal (token requis en prod)
      final resolvedToken =
          token ??
          await _tokenService.getRtcToken(
            channelId: channelId,
            uid: uid ?? 0,
          ) ??
          '';
