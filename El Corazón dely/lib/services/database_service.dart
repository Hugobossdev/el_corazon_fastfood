...
class DatabaseService {
  ...
  /// Assure la présence du profil dans `public.users` pour l'utilisateur connecté.
  /// Utile quand d'autres tables ont des FK vers `public.users.id`.
  Future<void> ensureUserProfile() async {
    try {
      ...
      final role = (meta['role'] ?? 'delivery').toString();

      // IMPORTANT:
      // - dans ce projet, `public.users.id` est un UUID interne (différent de `auth.users.id`)
      // - on "link" via `auth_user_id` (unique), donc on upsert sur `auth_user_id`
  ...
  /// Retourne la ligne `public.users` correspondant à l'utilisateur authentifié,
  /// en la créant si nécessaire.
  ///
  /// Utile pour obtenir le vrai `public.users.id` (FK), différent de `auth.users.id`.
  Future<Map<String, dynamic>?> ensureUserProfileExists() async {
    final authUser = _supabase.auth.currentUser;
    ...
    try {
      ...
      try {
        ...
        debugPrint('ensureUserProfileExists fallback: $e2');
        return null;
      }
    }
  }
  ...
  // =====================================================
  // DELIVERY FEE (source: orders)
  // =====================================================

  /// Récupère le dernier `delivery_fee` enregistré dans `orders` pour un utilisateur.
  /// Utile pour ré-hydrater le panier après refresh/reconnexion.
  Future<double?> getLastDeliveryFeeForUser(String userId) async {
    try {
      final row = await _supabase
          .from('orders')
          .select('delivery_fee')
      ...
      debugPrint('getLastDeliveryFeeForUser: $e');
      return null;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getAvailableOrders() async {
    try {
      ...
      // - Orders must have status: ready (la commande doit être prête avant assignation livreur)
      ...
      final response = await _supabase.from('orders').select('''
            *,
            order_items(*)
          ''').eq('status', 'ready').order('created_at', ascending: false);
  ...
  /// Historique des commandes d'un livreur (inclut les livraisons terminées).
  ///
  /// IMPORTANT: dans ce projet, `delivery_person_id` stocke l'id de `public.users.id`
  /// (pas `auth.users.id`). `AppService._currentUser.id` correspond à `public.users.id`.
  Future<List<Map<String, dynamic>>> getDriverOrderHistory(
    String deliveryId, {
    int limit = 100,
  }) async {
    try {
      ...
      debugPrint('❌ Error in getDriverOrderHistory: $e');
      throw Exception(
        'Erreur lors de la récupération de l\'historique des livraisons: $e',
      );
    }
  }
