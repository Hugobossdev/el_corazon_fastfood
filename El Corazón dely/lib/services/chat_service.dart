...
class ChatService extends ChangeNotifier {
  ...
  Future<bool> sendMessage({
    ...
    try {
      // IMPORTANT: D'après fix_chat_schema.sql, messages.sender_id référence auth.users(id)
      // Donc on doit utiliser l'auth_user_id, pas l'ID de la table public.users
      ...
      // S'assurer que le profil existe dans la table users pour récupérer le nom
      final ensuredProfile = await _databaseService.ensureUserProfileExists();
      final effectiveSenderName =
          (ensuredProfile?['name'] as String?) ?? senderName;

      debugPrint('ChatService: Sending message with sender_id (auth.users.id): $authUserId');

      // Envoyer le message via DatabaseService avec l'auth_user_id
      await _databaseService.sendMessage(
        orderId: orderId,
        senderId: authUserId, // Utiliser auth_user_id car la FK pointe vers auth.users(id)
      ...
      if (e.toString().contains('foreign key constraint')) {
        debugPrint('ChatService: Foreign key constraint violation - sender_id does not exist');
        final authUser = _supabase.auth.currentUser;
        debugPrint('ChatService: Auth user ID: ${authUser?.id}');
        debugPrint('ChatService: Provided senderId: $senderId');
      }
      return false;
    }
  }
