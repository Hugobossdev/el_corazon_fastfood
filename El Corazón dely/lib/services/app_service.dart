...
class AppService extends ChangeNotifier {
  ...
  List<Order> _orders = [];
  List<Order> _driverOrderHistory = [];
  ...
  List<Order> get driverOrderHistory => _driverOrderHistory;
  ...
  List<String> get categories {
    // Pas de valeurs "en dur": si aucun menu chargé, renvoyer une liste vide.
    if (_menuItems.isEmpty) return const [];
  ...
  Future<void> initialize() async {
    try {
      ...
      if (currentAuthUser != null) {
        ...
        // Hydrate delivery fee from `orders` so it survives refresh/re-login.
        if (_currentUser != null) {
          await CartService().hydrateDeliveryFeeFromOrders(_currentUser!.id);
        }
  ...
  Future<bool> login(String email, String password, UserRole role) async {
    try {
      ...
      if (response?.user != null) {
        ...
        // IMPORTANT: recharger les données depuis Supabase à la connexion
        await _loadUserOrders();
        if (_currentUser != null) {
          await CartService().hydrateDeliveryFeeFromOrders(_currentUser!.id);
        }
        if (_currentUser?.role == UserRole.delivery) {
          await Future.wait([
            ...
            loadDriverHistory(forceRefresh: true),
          ]);
        } else {
          _driverOrderHistory = [];
        }
  ...
  Future<String> placeOrder(
    ...
    try {
      ...
      // Le frais est calculé côté app (CartService) et doit être persisté sur la commande.
  ...
  Future<void> loadAvailableOrders({
    ...
    try {
      // Charger les commandes disponibles (optionnel)
      final List<Order> availableOrders = [];
      if (includeAvailable) {
        final ordersData = await _databaseService.getAvailableOrders().timeout(
          const Duration(seconds: 15),
        );
        availableOrders.addAll(ordersData.map((data) => Order.fromMap(data)));
      }
      ...
      notifyListeners();
      debugPrint(
        '✅ Loaded ${availableOrders.length} available orders (includeAvailable=$includeAvailable)',
      );
      debugPrint('✅ Loaded ${assignedOrders.length} assigned orders');
  ...
  bool _isLoadingDriverHistory = false;
  DateTime? _lastDriverHistoryLoadTime;

  Future<void> loadDriverHistory({bool forceRefresh = false}) async {
    if (_currentUser == null || _currentUser?.role != UserRole.delivery) {
      _driverOrderHistory = [];
      return;
    }
    if (_isLoadingDriverHistory) return;

    // cache très court pour éviter spam, mais toujours refreshable
    if (!forceRefresh &&
        _lastDriverHistoryLoadTime != null &&
        DateTime.now().difference(_lastDriverHistoryLoadTime!) <
            const Duration(seconds: 10)) {
      return;
    }

    _isLoadingDriverHistory = true;
    _lastDriverHistoryLoadTime = DateTime.now();
    try {
      final data = await _databaseService
          .getDriverOrderHistory(_currentUser!.id, limit: 200)
          .timeout(const Duration(seconds: 15));
      _driverOrderHistory = data.map((e) => Order.fromMap(e)).toList();
      _driverOrderHistory.sort((a, b) => b.createdAt.compareTo(a.createdAt));
      debugPrint(
        '✅ Loaded ${_driverOrderHistory.length} driver history orders',
      );
      notifyListeners();
    } catch (e) {
      debugPrint('❌ Error loading driver history: $e');
      // ne pas vider si on a déjà des données
      if (_driverOrderHistory.isEmpty) {
        _driverOrderHistory = [];
        notifyListeners();
      }
    } finally {
      _isLoadingDriverHistory = false;
    }
  }
    ...
    try {
      ...
      try {
        await _databaseService
            .updateOrderStatus(orderId, newStatus.dbValue)
            .timeout(const Duration(seconds: 10));
      } catch (e) {
  ...
  Future<void> acceptDelivery(String orderId) async {
    ...
    try {
      ...
      await _databaseService.assignOrderToDriver(orderId, _currentUser!.id);
    ...
    try {
      ...
      if (response?.user != null) {
        ...
        if (_currentUser?.role == UserRole.delivery) {
          ...
          // Recharger données livreur (actives + historique)
          await Future.wait([
            loadAvailableOrders(forceRefresh: true),
            loadDriverHistory(forceRefresh: true),
          ]);
