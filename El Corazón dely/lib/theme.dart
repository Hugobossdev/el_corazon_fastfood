...
RoundedRectangleBorder _appCardShape([double radius = 16]) {
  return RoundedRectangleBorder(borderRadius: BorderRadius.circular(radius));
}

InputDecorationTheme _inputDecorationTheme(ColorScheme scheme) {
  final outline = scheme.outline.withValues(alpha: 0.25);
...
ElevatedButtonThemeData _elevatedButtonTheme(ColorScheme scheme) {
  return ElevatedButtonThemeData(
    style: ElevatedButton.styleFrom(
      // IMPORTANT: Size.fromHeight(50) == Size(infinity, 50) which breaks
      // when the button is laid out under unbounded width constraints (e.g. inside Row).
...
OutlinedButtonThemeData _outlinedButtonTheme(ColorScheme scheme) {
  return OutlinedButtonThemeData(
    style: OutlinedButton.styleFrom(
      // Same rationale as above: avoid infinite width under unbounded constraints.
      minimumSize: const Size(0, 50),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
...
ThemeData get lightTheme => ThemeData(
  ...
  dividerTheme: DividerThemeData(
    space: 1,
    thickness: 1,
    color: LightModeColors.lightShadow.withValues(alpha: 0.08),
  ),
  textTheme: TextTheme(
