...
class AppService extends ChangeNotifier {
  ...
  String _cleanAddressString(String address) {
    ...
    cleaned = cleaned.replaceAll(RegExp(r'[\u{1F300}-\u{1F9FF}]', unicode: true), ''); // Emojis
    cleaned = cleaned.replaceAll(RegExp(r'[\u{2600}-\u{26FF}]', unicode: true), ''); // Symboles divers
    cleaned = cleaned.replaceAll(RegExp(r'[\u{2700}-\u{27BF}]', unicode: true), ''); // Symboles Dingbats
    cleaned = cleaned.replaceAll(RegExp(r'[\u{1F600}-\u{1F64F}]', unicode: true), ''); // Emojis visages
    cleaned = cleaned.replaceAll(RegExp(r'[\u{1F680}-\u{1F6FF}]', unicode: true), ''); // Emojis transport
    cleaned = cleaned.replaceAll(RegExp(r'[\u{1F900}-\u{1F9FF}]', unicode: true), ''); // Emojis supplémentaires
    ...
    cleaned = cleaned.replaceAll(RegExp(r'[^a-zA-Z0-9\s,\-\.\(\)àáâãäåèéêëìíîïòóôõöùúûüýÿçñÀÁÂÃÄÅÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝŸÇÑ]'), '');
  ...
  Future<String> placeOrderFromCartService(
    ...
    try {
      ...
      // Create order data for database
      // Nettoyer l'adresse pour supprimer les emojis et caractères non autorisés
      final addressString = _cleanAddressString(addressForValidation.fullAddress);
      final orderData = {
        'id': orderId,
        'user_id': _currentUser!.id,
        'status': 'pending',
      ...
      if (paymentMethod == PaymentMethod.mobileMoney) {
        ...
        throw Exception('Le portefeuille électronique est temporairement indisponible');
      ...
      for (final dynamic item in cartItems) {
        ...
        if (item is! CartItem) {
          debugPrint('Warning: Item is not a CartItem, skipping: ${item.runtimeType}');
          continue;
        }
        ...
        try {
          ...
          final existingMenuItem = await _databaseService.getMenuItemById(menuItemId);
          if (existingMenuItem == null) {
            debugPrint('Warning: Menu item $menuItemId does not exist, creating it...');
