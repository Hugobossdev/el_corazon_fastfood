...
import 'package:elcora_fast/utils/security_helper.dart' show SecurityHelper, SecurityException;
...
class DatabaseService {
  ...
  Future<AuthResponse?> signUp({
    ...
    try {
      ...
      if (sanitizedEmail == null || !InputSanitizer.isValidEmailSafe(sanitizedEmail)) {
        throw Exception('L\'email contient des caractères non autorisés ou est invalide');
      }
      ...
      if (sanitizedPhone == null) {
        throw Exception('Le numéro de téléphone contient des caractères non autorisés ou est invalide');
      }

      final response = await _supabase.auth.signUp(
        email: sanitizedEmail,
        password: password, // Le mot de passe n'est pas sanitizé (caractères spéciaux autorisés)
        data: {
          'name': sanitizedName,
  ...
  Future<AuthResponse?> signIn({
    ...
    try {
      ...
      // S'assurer qu'un profil public.users existe pour éviter les FK (messages_sender_id_fkey, etc.)
  ...
  Future<void> ensureUserProfile() async {
    try {
      ...
      final name = (meta['name'] ?? meta['full_name'] ?? 'Utilisateur').toString();
  ...
  Future<Map<String, dynamic>?> getUserProfile(String userId) async {
    try {
      ...
      final byAuth = await _supabase
          ...
          .maybeSingle();
      if (byAuth != null) {
        debugPrint('DatabaseService.getUserProfile: Found user by auth_user_id: $userId');
        return byAuth;
      }
  ...
  Future<Map<String, dynamic>?> ensureUserProfileExists(
    ...
    if (targetId == null) {
      debugPrint('DatabaseService.ensureUserProfileExists: No target ID provided');
      return null;
    }
    ...
    if (existing != null) {
      ...
      debugPrint('DatabaseService.ensureUserProfileExists: User already exists');
      ...
      if (authUserIdFromTable != targetId) {
        debugPrint('DatabaseService.ensureUserProfileExists: WARNING - Found user but auth_user_id does not match!');
        debugPrint('  - Expected auth_user_id: $targetId');
        debugPrint('  - Found auth_user_id: $authUserIdFromTable');
        debugPrint('  - This is likely a different user. Will try to create new profile.');
        ...
        if (userIdFromTable == null) {
          debugPrint('DatabaseService.ensureUserProfileExists: WARNING - users.id is null!');
          return null;
        }
    ...
    if (currentAuthUser == null || currentAuthUser.id != targetId) {
      debugPrint('DatabaseService.ensureUserProfileExists: Cannot create profile for different user');
      return null;
    }
    ...
    try {
      debugPrint('DatabaseService.ensureUserProfileExists: Creating user profile for auth_user_id: $targetId');
      ...
      if (inserted != null) {
        debugPrint('DatabaseService.ensureUserProfileExists: User profile created with ID: ${inserted['id']}');
        return inserted;
      }
      ...
      if (retrieved != null) {
        debugPrint('DatabaseService.ensureUserProfileExists: User profile retrieved with ID: ${retrieved['id']}');
        return retrieved;
      }

      debugPrint('DatabaseService.ensureUserProfileExists: Failed to create or retrieve user profile');
      return null;
    } catch (e) {
      ...
      try {
        final retrieved = await getUserProfile(targetId);
        if (retrieved != null) {
          debugPrint('DatabaseService.ensureUserProfileExists: User profile found after error with ID: ${retrieved['id']}');
          return retrieved;
        }
      } catch (retrieveError) {
        debugPrint('DatabaseService.ensureUserProfileExists: Error retrieving profile after insert error: $retrieveError');
      }
      return null;
    }
  }
  ...
  Future<String> createOrder(Map<String, dynamic> orderData) async {
    try {
      ...
      final sanitizedData = SecurityHelper.sanitizeData(
        orderData,
        excludeFields: ['total', 'subtotal', 'delivery_fee', 'discount'], // Exclure les champs numériques
      );
